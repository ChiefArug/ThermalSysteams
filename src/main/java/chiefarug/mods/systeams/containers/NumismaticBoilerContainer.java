...
public class NumismaticBoilerContainer extends BoilerContainerBase<NumismaticBoilerBlockEntity> {
	public NumismaticBoilerContainer(int windowId, Inventory inv, FriendlyByteBuf data) {
		this(windowId, ProxyUtils.getClientWorld(), data.readBlockPos(), inv, ProxyUtils.getClientPlayer());
	}

	public NumismaticBoilerContainer(int windowId, Level level, BlockPos pos, Inventory inventory, Player player) {
		super(SysteamsRegistry.Boilers.NUMISMATIC.menu(), windowId, level, pos, inventory, player);
