...
public class StirlingBoilerMenu extends BoilerMenuBase<StirlingBoilerBlockEntity> {

	public StirlingBoilerMenu(int windowId, Inventory inv, FriendlyByteBuf data) {
		this(windowId, ProxyUtils.getClientWorld(), data.readBlockPos(), inv, ProxyUtils.getClientPlayer());
	}

	public StirlingBoilerMenu(int windowId, Level level, BlockPos pos, Inventory inventory, Player player) {
