...
public class GourmandBoilerMenu extends BoilerMenuBase<GourmandBoilerBlockEntity> {

	public GourmandBoilerMenu(int windowId, Inventory inv, FriendlyByteBuf data) {
		this(windowId, ProxyUtils.getClientWorld(), data.readBlockPos(), inv, ProxyUtils.getClientPlayer());
	}

	public GourmandBoilerMenu(int windowId, Level level, BlockPos pos, Inventory inventory, Player player) {
